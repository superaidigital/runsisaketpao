...
// สคริปต์สำหรับจัดการ (เพิ่ม/แก้ไข/ลบ) บัญชีเจ้าหน้าที่
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    if ($action === 'update') {
        ...
        if ($staff_id === 0 || empty($full_name)) {
            $_SESSION['update_error'] = "ข้อมูลไม่ครบถ้วน";
        } else {
            if (!empty($password)) {
                ...
                if (strlen($password) < 6) {
                    ...
                    $password_hash = password_hash($password, PASSWORD_DEFAULT);
                    $stmt = $mysqli->prepare("UPDATE staff SET full_name = ?, role = ?, assigned_event_id = ?, password_hash = ? WHERE id = ?");
                    $stmt->bind_param("ssisi", $full_name, $role, $assigned_event_id, $password_hash, $staff_id);
                }
            } else {
                // Update without changing password
                $stmt = $mysqli->prepare("UPDATE staff SET full_name = ?, role = ?, assigned_event_id = ? WHERE id = ?");
                $stmt->bind_param("ssii", $full_name, $role, $assigned_event_id, $staff_id);
            }
            
            if (isset($stmt) && $stmt->execute()) {
                $_SESSION['update_success'] = "อัปเดตข้อมูลบัญชีสำเร็จ";
            } else if (!isset($_SESSION['update_error'])) {
                 $_SESSION['update_error'] = "เกิดข้อผิดพลาดในการอัปเดตข้อมูล";
            }
